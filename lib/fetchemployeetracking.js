...
module.exports = {
    fetchemployeetracking: function(req,res){
        try{
            ...
            mysql.query("SELECT  DISTINCT employeetrackingmaster.Id, case when employeetrackingmaster.ToDate = '0000-00-00' then 1 when isnull(employeetrackingmaster.ToDate) then 1 else 0 end as Action , employeetrackingmaster.UserId,employeetrackingmaster.type,employeemaster.EmployeeId,employeemaster.FirstName,employeemaster.LastName,employeemaster.ReportingManager AS Reporting_Head,employeemaster.reportsToName AS  reportingmanager,employeemaster.Branch AS Branch,employeemaster.integraExperience,employeemaster.totalExperience,employeetrackingmaster.ProjectName,employeetrackingmaster.CompanyName,CONVERT(employeetrackingmaster.FromDate,CHAR CHARACTER SET utf8) AS FromDate,CONVERT(employeetrackingmaster.ToDate,CHAR CHARACTER SET utf8) AS ToDate from employeetrackingmaster JOIN employeemaster ON employeemaster.UserId = employeetrackingmaster.UserId  WHERE employeetrackingmaster.IsDeleted='0' and employeetrackingmaster.UserId = "+ mysql.escape(UserId),function(err, results) {
